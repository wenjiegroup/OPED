...
my $chr='chr1';	#染色体 
my $start_position=943995;	#染色体上的起始位置 
my $edit_pattern='-GAGAACTCGG';	#编辑模式, /T表示将当前碱基替换成T, +GTATT表示在当前碱基后插入GTATT，-GAGAACTCGG表示在当前碱基后删除子序列GAGAACTCGG
...
elsif($edit_pattern=~/^-[acgtACTG]+$/)
{
	...
	$temp=~s/^-//;
	die("The following reference sequence of $chr:$start_position is ".substr($user_seq,$length+1,20).", but the deleted sequence $temp does not match it.") unless uc(substr($user_seq,$length+1,length($temp))) eq uc($temp);
...
sub extract_sequence
{
	...
	my $num_colunm=50;
	open(FILE,"<","../Reference_Genome/$chr.fa") or die("open file error!\n$!");
