...
class PositionalEncoding(nn.Module):
    r"""无参数的PositionEncoding计算速度快，还可以减小整个模型的尺寸，据说在有些任务中，效果与有参数的接近。
...
class LearnedPositionEncoding(nn.Embedding):
    """对于有些任务，带有参数的PositionEncoding层效果明显比没有参数的PositionEncoding要好。
    带参数的PositionEncoding层的定义更为简单，直接继承一个nn.Embedding，再续上一个dropout就可以了。
